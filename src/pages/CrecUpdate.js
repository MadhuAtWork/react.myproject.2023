...
const headCells = [
    {
        id: 'meetingCenterCode',
        label: 'Metting Center Code',
    },
    {
...
function CrecUpdate() {
    ...
    const fetchMeetingCenterURL = '/fetchMeetingCenter';
    const bulkUpdateCRECURL = '/bulkCrecUpdate';
    ...
    const handleCurrentCrecUpdate = async (event) => {
        ...
        const requestOptionscrecbulk = {
            ...
            body: JSON.stringify({
                "crecName": inputCrecName
            }),
        };
        await fetch(baseURL + fetchMeetingCenterURL, requestOptionscrecbulk)
    ...
    const onHandleCrecBulkUpdate = async (event) => {
        ...
        if (inputCrecName === inputNewCrecName) {
            alert("You can not transfer CREC to same employee.")
            return;
        }
        ...
        if (inputNewCrecName == "") {
            alert("Please select new CREC employee number to transfer selected Meeting Centers")
            return;
        }
        ...
        const requestOptionscrecbulk = {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                "meetingCenterCodeList": finalListStr,
                "crecName": inputCrecName,
                "newCrecName": inputNewCrecName
            }),
        };
        await fetch(baseURL + bulkUpdateCRECURL, requestOptionscrecbulk)
            .then(response => response.json())
            .then((response) => {
                console.log(response)
                setSelected([]);
  ...
  const onHandleCrecUpDateCancel = () =>{
                        ...
                        <Box sx={{ width: '100%' }}>
                            ...
                            <Paper sx={{ width: '100%' }}>
                                ...
                                <div className='d-flex justify-content-end p-3' >
                                    ...
                                    >Update</Button>
                                    <Button type="button" className='bg-orenge ms-3' variant="container" onClick={onHandleCrecUpDateCancel} >
                                        <p>Cancel</p>
                                    </Button>
                                </div>
                            </Paper>
